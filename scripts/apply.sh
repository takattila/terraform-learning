...
function import_resource_group_if_needed() {
    # Import the resource group if not already imported this run
...
function extract_and_import() {
    ...
    # Extract resource IDs and names from error messages for import
...
function main() {
    if [ -z "$MODULE" ] || [ -z "$ARM_SUBSCRIPTION_ID" ]; then
        echo "Usage: $0 <MODULE> <ARM_SUBSCRIPTION_ID>"
        exit 1
    fi

    RG_IMPORTED="false"  # Flag to track resource group import status
    ...
    while true; do
        ...
        # Check if apply succeeded (no "Error" in output)
        if ! echo "$apply_output" | grep -q "Error"; then
            echo "=== Terraform apply completed successfully. ==="
            break
        fi

        # If "already exists" error found, import resources, including resource group once
